...
# API, number of results, minimum similarity
sauce = SauceNao(api_key=api, results_limit=5, min_similarity=60)


path = os.environ.get("user_path")  # working directory where all images
# should be placed in before script runs
os.chdir(path)
...
def danbooru_source(url, index):
    ...
    if parsed_url[1] == 'twitter.com':
        ...
        new_url = urllib.parse.urlunparse(
            parsed_url._replace(path=(r'/' + user)))
        ...
        try:
            ...
            if len(source) > 0:
                ...
                if empty_string(author_name):
                    ...
                    folder_moving(image, author_name, title +
            ...
            folder_moving(image, r"~problems\~Can't retrieve from SauceNAO",
                          'returned_error' + os.path.splitext(image)[1])
            pass
